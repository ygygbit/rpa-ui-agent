"""
Experiment 18: Step Budget Awareness

Tests whether telling the VLM how many steps it has used/remaining
makes it more efficient (fewer steps, faster completion).

Hypothesis: VLM will be more decisive and efficient when it knows
its step budget, especially on longer tasks where it might otherwise
dally or over-verify.

Configs:
  A (baseline): step_budget_awareness=False (no budget info)
  B (budget): step_budget_awareness=True (inject step X/Y + urgency)

Usage:
    python test_exp18_step_budget.py
"""
...
def run_single_test(task_info: dict, config_label: str,
    ...
    config = AgentConfig(
        ...
        step_budget_awareness=step_budget,
    )
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 18: Step Budget Awareness[/]\n"
        "Compares baseline (no budget info) vs step_budget_awareness=True\n"
        "Tests whether knowing step budget makes VLM more efficient",
        border_style="blue",
    ))
    ...
    # --- Config A: Baseline ---
    console.print(Panel(
        "[bold]Config A: Baseline[/] (step_budget_awareness=False)",
        border_style="yellow",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "baseline", step_budget=False)
        results.append(result)

    # --- Config B: Step Budget ---
    console.print(Panel(
        "[bold]Config B: Step Budget[/] (step_budget_awareness=True)",
        border_style="green",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "budget", step_budget=True)
        results.append(result)

    # ---- Comparison Table ----
    console.print("\n")
    console.print(Panel("[bold]Experiment 18 Results[/]", border_style="cyan"))

    table = Table(title="Experiment 18: Step Budget Awareness")
    ...
    # ---- Save results ----
    output_path = Path("exp18_step_budget_results.json")
