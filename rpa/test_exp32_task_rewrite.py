"""
Experiment 32: Auto-Navigate with Task Rewrite

Tests the combination of auto-navigate (HTTP API navigation before VLM loop)
with task rewriting (removing "Go to X.com" from task, replacing with
"The browser is already on X.com. <remaining task>").

This addresses Exp 31's weakness where the VLM still saw "Go to X.com" in the
task even though it was already there, causing confusion on complex tasks.

Configs:
  A (baseline): auto_navigate=False (VLM navigates, adaptive_prompt=True)
  B (auto-nav + rewrite): auto_navigate=True, task rewritten to remove nav

Usage:
    python test_exp32_task_rewrite.py
"""
...
def run_single_test(task_info: dict, config_label: str,
    ...
    # Preview what the rewritten task would be
    rewritten = ""
    if auto_nav:
        url = agent._extract_target_url(task_text)
        if url:
            rewritten = agent._rewrite_task_after_navigate(task_text, url)
            console.print(f"  [cyan]Will rewrite to: {rewritten[:100]}...[/]")
    ...
    result = TaskResult(
        ...
        rewritten_task=rewritten if rewritten else "none",
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 32: Auto-Navigate + Task Rewrite[/]\n"
        "Tests auto-navigate with task rewriting vs baseline\n"
        "Both configs use adaptive_prompt=True (Exp 30 baseline)",
        border_style="blue",
    ))
    ...
    # --- Config A: Baseline ---
    console.print(Panel(
        "[bold]Config A: Baseline[/] (auto_navigate=False, adaptive_prompt=True)",
        border_style="yellow",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "baseline", auto_nav=False)
        results.append(result)

    # --- Config B: Auto-Navigate + Rewrite ---
    console.print(Panel(
        "[bold]Config B: Auto-Navigate + Task Rewrite[/] (auto_navigate=True)",
        border_style="green",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "auto_rewrite", auto_nav=True)
        results.append(result)

    # ---- Comparison Table ----
    console.print("\n")
    console.print(Panel("[bold]Experiment 32 Results[/]", border_style="cyan"))

    table = Table(title="Experiment 32: Auto-Navigate + Task Rewrite")
    ...
    for label in ["baseline", "auto_rewrite"]:
    ...
    # ---- Per-task delta ----
    console.print("\n")
    delta_table = Table(title="Per-Task Step Comparison (Baseline vs Auto-Rewrite)")
    delta_table.add_column("Task", style="bold")
    delta_table.add_column("Baseline Steps", justify="right")
    delta_table.add_column("Auto-Rewrite Steps", justify="right")
    delta_table.add_column("Delta", justify="right")
    delta_table.add_column("Rewritten Task")

    for task_info in TASKS:
        name = task_info["name"]
        baseline = next((r for r in results if r.task_name == name and r.config_label == "baseline"), None)
        auto = next((r for r in results if r.task_name == name and r.config_label == "auto_rewrite"), None)
        if baseline and auto:
            delta = auto.step_count - baseline.step_count
            ...
            delta_table.add_row(
                ...
                str(auto.step_count),
                delta_str,
                auto.rewritten_task[:50] + "..." if len(auto.rewritten_task) > 50 else auto.rewritten_task,
            )

    console.print(delta_table)

    # ---- Save results ----
    output_path = Path("exp32_task_rewrite_results.json")
