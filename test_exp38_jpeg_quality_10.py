"""
Experiment 38: JPEG Quality Extreme Floor (q25 vs q10)

Tests whether JPEG quality 10 still works or if q25 is the floor.
Exp 36 showed q75→q50 works, Exp 37 showed q50→q25 works.
This pushes to q10 — severe blocking artifacts expected.

At q10, JPEG blocking artifacts are very visible. Small text like
grid coordinate labels may become unreadable. This should find the floor.

Configs:
  A (baseline): vlm_image_quality=25 (current default after Exp 37)
  B (extreme):  vlm_image_quality=10

Usage:
    python test_exp38_jpeg_quality_10.py
"""
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 38: JPEG Quality Extreme Floor (q25 vs q10)[/]\n"
        "Tests if q10 still works or if q25 is the floor\n"
        "Resolution stays at 1024px. Expect severe blocking artifacts at q10",
        border_style="blue",
    ))
    ...
    # --- Config A: q25 (current default) ---
    console.print(Panel(
        "[bold]Config A: q25[/] (current default)",
        border_style="yellow",
    ))
    ...
    # --- Config B: q10 ---
    console.print(Panel(
        "[bold]Config B: q10[/] (extreme compression)",
        border_style="green",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "q10", quality=10)
        results.append(result)

    # ---- Comparison Table ----
    console.print("\n")
    console.print(Panel("[bold]Experiment 38 Results[/]", border_style="cyan"))

    table = Table(title="Experiment 38: JPEG Quality Extreme (q25 vs q10)")
    ...
    for task_info in TASKS:
        ...
        rq10 = next((r for r in results if r.task_name == name and r.config_label == "q10"), None)
        if rq25 and rq10:
            step_delta = rq10.step_count - rq25.step_count
            ...
            q10_per = rq10.total_input_tokens // rq10.step_count if rq10.step_count else 0
            tok_delta_pct = (q10_per - q25_per) / q25_per * 100 if q25_per else 0
    ...
    # ---- Save results ----
    output_path = Path("exp38_jpeg_quality_10_results.json")
