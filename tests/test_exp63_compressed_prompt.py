"""
Experiment 63: Compressed System Prompt vs Standard

The standard prompt is ~7,432 chars (~1,858 tokens). A compressed version
with the same behavioral rules is ~2,007 chars (~501 tokens) — 73% smaller.

This saves ~1,357 tokens per VLM call. At 7 avg steps, that's ~9.5K fewer
tokens per task. The question is whether the compressed prompt maintains
the same success rate and step count.

Configs:
  A (standard): current default system prompt (~1,858 tokens)
  B (compressed): compressed system prompt (~501 tokens)

Usage:
    python test_exp63_compressed_prompt.py
"""
...
def run_single_test(task_info, config_label, system_prompt=None):
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 63: Compressed Prompt vs Standard[/]\n"
        "Standard: ~1,858 tokens | Compressed: ~501 tokens (73% smaller)\n"
    ...
    console.print(Panel("[bold]Config A: Standard prompt[/] (current default)", border_style="yellow"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "standard", system_prompt=None))

    console.print(Panel("[bold]Config B: Compressed prompt[/] (73% smaller)", border_style="green"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "compressed", system_prompt=SystemPrompts.GUI_AGENT_COMPRESSED))

    # Results table
    console.print("\n")
    table = Table(title="Experiment 63: Compressed Prompt vs Standard")
    ...
    for label in ["standard", "compressed"]:
    ...
    output_path = Path("exp63_compressed_prompt_results.json")
