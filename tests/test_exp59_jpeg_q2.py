"""
Experiment 59: JPEG Quality q2 vs q5

q5 worked perfectly (Exp 58). Testing q2 - extreme compression.
At q2, we expect severe JPEG artifacts. The question is whether
VLM can still interpret UI elements, grid labels, and text.

Configs:
  A (q5): current default vlm_image_quality=5
  B (q2): extreme compression vlm_image_quality=2

Usage:
    python test_exp59_jpeg_q2.py
"""
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 59: JPEG Quality q2 vs q5[/]\n"
    ...
    console.print(Panel("[bold]Config A: q5[/] (current default)", border_style="yellow"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "q5", vlm_image_quality=5))

    console.print(Panel("[bold]Config B: q2[/] (extreme compression)", border_style="green"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "q2", vlm_image_quality=2))

    # Results table
    console.print("\n")
    table = Table(title="Experiment 59: JPEG Quality q2 vs q5")
    ...
    output_path = Path("exp59_jpeg_q2_results.json")
