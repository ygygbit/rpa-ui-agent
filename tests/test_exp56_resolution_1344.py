"""
Experiment 56: VLM Resolution 1344px vs 1024px

The original resolution was 1344px (before Exp 5 optimized to 1024px).
Back then we used PNG format, so tokens were massive. Now with JPEG q10,
higher resolution might be affordable while giving VLM more pixel detail
for coordinate accuracy.

Hypothesis: With q10 JPEG, going from 1024px to 1344px adds ~70% pixels
but JPEG keeps token growth moderate. Better detail could help on complex
tasks requiring precise coordinate identification.

Configs:
  A (1024px): current default vlm_max_edge=1024
  B (1344px): higher resolution vlm_max_edge=1344

Usage:
    python test_exp56_resolution_1344.py
"""
...
def run_single_test(task_info, config_label, vlm_max_edge):
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 56: VLM Resolution 1344px vs 1024px[/]\n"
        "JPEG q10, grid_spacing=400, step_delay=0",
        border_style="blue",
    ))
    ...
    console.print(Panel("[bold]Config A: res_1024[/] (current default)", border_style="yellow"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "res_1024", vlm_max_edge=1024))

    console.print(Panel("[bold]Config B: res_1344[/] (higher resolution)", border_style="green"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "res_1344", vlm_max_edge=1344))

    # Results table
    console.print("\n")
    table = Table(title="Experiment 56: VLM Resolution 1344px vs 1024px")
    ...
    for label in ["res_1024", "res_1344"]:
    ...
    # Per-task delta
    dt = Table(title="Per-Task Comparison")
    dt.add_column("Task", style="bold")
    dt.add_column("1024 Steps", justify="right")
    dt.add_column("1344 Steps", justify="right")
    dt.add_column("Step Delta", justify="right")
    dt.add_column("1024 Tok/Step", justify="right")
    dt.add_column("1344 Tok/Step", justify="right")
    dt.add_column("Tok/Step %", justify="right")
    for task_info in TASKS:
        name = task_info["name"]
        ra = next((r for r in results if r.task_name == name and r.config_label == "res_1024"), None)
        rb = next((r for r in results if r.task_name == name and r.config_label == "res_1344"), None)
        if ra and rb:
            d = rb.step_count - ra.step_count
            ds = f"[green]{d}[/]" if d < 0 else f"[red]+{d}[/]" if d > 0 else "0"
            ta = ra.total_input_tokens // ra.step_count if ra.step_count else 0
            tb = rb.total_input_tokens // rb.step_count if rb.step_count else 0
            tp = ((tb - ta) / ta * 100) if ta else 0
            ts = f"[green]{tp:+.0f}%[/]" if tp < 0 else f"[red]{tp:+.0f}%[/]" if tp > 0 else "0%"
            dt.add_row(name, str(ra.step_count), str(rb.step_count), ds, f"{ta:,}", f"{tb:,}", ts)
    console.print(dt)

    output_path = Path("exp56_resolution_1344_results.json")
