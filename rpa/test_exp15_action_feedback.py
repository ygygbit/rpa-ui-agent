"""
Experiment 15: Action Confirmation Feedback

Tests whether injecting a brief confirmation message into conversation history
after successful actions improves VLM situational awareness and task completion.

Currently, the VLM only receives feedback when an action fails. On success,
the next step just gets a new screenshot with no context about what was just done.
This experiment adds a user message like "Action 'click' executed successfully.
Clicked at (500, 300)." after each successful action.

Configs:
  A (baseline): action_feedback=False (original behavior)
  B (feedback): action_feedback=True

Usage:
    python test_exp15_action_feedback.py
"""
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 15: Action Confirmation Feedback[/]\n"
        "Compares baseline (no feedback) vs action_feedback=True\n"
        "Tests whether confirming successful actions improves VLM awareness",
        border_style="blue",
    ))
    ...
    # --- Config A: Baseline ---
    console.print(Panel(
        "[bold]Config A: Baseline[/] (action_feedback=False)",
        border_style="yellow",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "baseline", action_feedback=False)
        results.append(result)

    # --- Config B: Action feedback enabled ---
    console.print(Panel(
        "[bold]Config B: Action Feedback[/] (action_feedback=True)",
        border_style="green",
    ))
    for task_info in TASKS:
        result = run_single_test(task_info, "feedback", action_feedback=True)
        results.append(result)

    # ---- Comparison Table ----
    console.print("\n")
    console.print(Panel("[bold]Experiment 15 Results[/]", border_style="cyan"))

    table = Table(title="Experiment 15: Action Confirmation Feedback")
    ...
    for label in ["baseline", "feedback"]:
    ...
    # ---- Save results ----
    output_path = Path("exp15_action_feedback_results.json")
