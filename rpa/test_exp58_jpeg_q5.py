"""
Experiment 58: JPEG Quality q5 vs q10

Progressive JPEG quality reduction: q75->q50->q25->q10 have all been positive.
This tests q5, the next step down. At some point, artifacts will degrade VLM
performance by making text/UI elements unreadable.

Now testing at 1344px resolution (from Exp 56 merge).

Configs:
  A (q10): current default vlm_image_quality=10
  B (q5): reduced vlm_image_quality=5

Usage:
    python test_exp58_jpeg_q5.py
"""
...
def run_single_test(task_info, config_label, vlm_image_quality):
    ...
    config = AgentConfig(
        ...
        vlm_image_quality=vlm_image_quality,
    )
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 58: JPEG Quality q5 vs q10[/]\n"
        "1344px, grid_spacing=400, step_delay=0",
        border_style="blue",
    ))
    ...
    console.print(Panel("[bold]Config A: q10[/] (current default)", border_style="yellow"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "q10", vlm_image_quality=10))

    console.print(Panel("[bold]Config B: q5[/] (reduced)", border_style="green"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "q5", vlm_image_quality=5))

    # Results table
    console.print("\n")
    table = Table(title="Experiment 58: JPEG Quality q5 vs q10")
    ...
    dt.add_column("q10 Steps", justify="right")
    dt.add_column("q5 Steps", justify="right")
    dt.add_column("Step Delta", justify="right")
    dt.add_column("q10 Tok/Step", justify="right")
    dt.add_column("q5 Tok/Step", justify="right")
    dt.add_column("Tok/Step %", justify="right")
    for task_info in TASKS:
        name = task_info["name"]
        ra = next((r for r in results if r.task_name == name and r.config_label == "q10"), None)
        rb = next((r for r in results if r.task_name == name and r.config_label == "q5"), None)
        if ra and rb:
    ...
    output_path = Path("exp58_jpeg_q5_results.json")
