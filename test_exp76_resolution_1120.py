"""
Experiment 76: Resolution 1120px vs 1344px

Current default vlm_max_edge=1344. Testing 1120px which should
significantly reduce image tokens (area-based scaling). The VLM
may still be accurate enough without grid overlay at this resolution.

1344px -> ~1344x756 = ~1,016K pixels
1120px -> ~1120x630 = ~706K pixels (-30% area)

Configs:
  A (1344): current default vlm_max_edge=1344
  B (1120): vlm_max_edge=1120

Usage:
    python test_exp76_resolution_1120.py
"""
...
def run_single_test(task_info, config_label, max_edge=1344):
...
def main():
    console.print(Panel.fit(
        "[bold blue]Experiment 76: Resolution 1120px vs 1344px[/]\n"
        "Testing lower resolution for fewer image tokens",
        border_style="blue",
    ))
    ...
    console.print(Panel("[bold]Config A: vlm_max_edge=1344[/] (current default)", border_style="yellow"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "1344px", max_edge=1344))

    console.print(Panel("[bold]Config B: vlm_max_edge=1120[/]", border_style="green"))
    for task_info in TASKS:
        results.append(run_single_test(task_info, "1120px", max_edge=1120))

    # Results table
    console.print("\n")
    table = Table(title="Experiment 76: Resolution 1120px vs 1344px")
    ...
    output_path = Path("exp76_resolution_1120_results.json")
